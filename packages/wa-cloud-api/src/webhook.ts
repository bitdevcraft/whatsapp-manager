...
export default class WebhookHandler {
  ...
  private postProcessHandler:
    | ((whatsapp: WhatsApp, message: WebhookMessage) => void | Promise<void>)
    | null = null;
  private flowHandlers: Map<
    FlowTypeEnum,
    (whatsapp: WhatsApp, request: FlowEndpointRequest) => any | Promise<any>
  > = new Map();
   ...
   */
  public handleVerificationRequest<Req extends IRequest, Res extends IResponse>(
    req: Req,
    res: Res
  ): void {
    ...
    if (
      mode === "subscribe" &&
      token === this.config.WEBHOOK_VERIFICATION_TOKEN
    ) {
      ...
      for (const message of messages) {
        ...
        if (this.preProcessHandler) {
          await Promise.resolve(
            this.preProcessHandler(this.client, processedMessage)
          );
        }
        ...
        if (this.postProcessHandler) {
          await Promise.resolve(
            this.postProcessHandler(this.client, processedMessage)
          );
        }
      }
    }
  ...
  public async handleFlowRequest<
    Req extends IRequest & { rawBody: string },
    Res extends IResponse,
  >(req: Req, res: Res): Promise<void> {
    ...
    if (flowType && this.flowHandlers.has(flowType)) {
      ...
      if (handler) {
        const response = await Promise.resolve(
          handler(this.client, decryptedBody)
        );
    ...
    LOGGER.log(
      "No handler found, sending an empty response body: ",
      decryptedBody
    );
    const encryptedResponse = this.encryptResponse(
      {},
  ...
  public isRequestSignatureValid(req: IRequest & { rawBody: string }): boolean {
    ...
    if (!appSecret) {
      LOGGER.warn(
        "App Secret is not set up. Please Add M4D_APP_SECRET or configuration"
      );
      return false;
    }
