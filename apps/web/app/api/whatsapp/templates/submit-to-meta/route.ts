...
export async function POST(request: Request) {
  try {
    ...
    console.log('Submitting template to Meta with data:', JSON.stringify(existingTemplate, null, 2));
    ...
    try {
      ...
      const updatedTemplate = await withTenantTransaction(teamId, async (tx) => {
...
async function submitTemplateToMeta(
  ...
  if (!accessToken || !businessAccountId) {
    throw new Error(`Missing required configuration. Access Token: ${accessToken ? 'Present' : 'Missing'}, Business Account ID: ${businessAccountId ? 'Present' : 'Missing'}`);
  }
